                         ...
                         struct device *dev)
{
	...
	if(cool==1){
	 for (j = 0; j < cool_num*(trip_num-1); ++j){
	...
	for (i = 0; i < trip_num -1; ++i){
			...
			if(cool==1){
				for (k = 0; k < cool_num; ++k){
					memcpy(pdata->trip_points[i].cdev_name[k],
							point_arr[i+k*(trip_num-1)], strlen(point_arr[i+k*(trip_num-1)])+1);
