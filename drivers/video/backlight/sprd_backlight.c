...
static inline uint32_t pwm_read(unsigned long index, unsigned long reg)
{
...
static void pwm_write(unsigned long index, uint32_t value, unsigned long reg)
{
	//this is the D-die PWM controller, here we use PWM2(index=3) for external backlight control.
...
static void srpd_backlight_init(void)
{
	...
	sci_adi_set((unsigned long)ANA_REG_GLB_WHTLED_CTRL1, BIT_PWM0_EN );

	//this is the A-die PWM controller, it is used for white_led.
	sci_adi_raw_write((unsigned long)PD_PWM_BASE+PWM_CNT, PWM_DUTY | PWM_MOD);
	sci_adi_raw_write((unsigned long)PD_PWM_BASE+PWM_TONE_DIV, PWM_DIV);
	sci_adi_raw_write((unsigned long)PD_PWM_BASE+PWM_PRESCALE, PWM_ENABLE | PWM_SCALE);
	sci_adi_raw_write((unsigned long)PD_PWM_BASE+PWM_PAT_LOW, PWM_LOW);
	sci_adi_raw_write((unsigned long)PD_PWM_BASE+PWM_PAT_HIG, PWM_HIG);

	sci_adi_set((unsigned long)ANA_REG_GLB_WHTLED_CTRL0, BIT_WHTLED_SERIES_EN);
	sci_adi_set((unsigned long)ANA_REG_GLB_WHTLED_CTRL0, BIT_WHTLED_BOOST_EN);
	sci_adi_set((unsigned long)ANA_REG_GLB_WHTLED_CTRL0, BIT_WHTLED_PD);

	sci_adi_set((unsigned long)ANA_REG_GLB_WHTLED_CTRL1, BIT_RTC_PWM0_EN);
	sci_adi_set((unsigned long)ANA_REG_GLB_WHTLED_CTRL1, BIT_PWM0_EN);
}
#endif
...
static int sprd_bl_whiteled_update_status(struct backlight_device *bldev)
{
	...
	if ((bldev->props.state & (BL_CORE_SUSPENDED | BL_CORE_FBBLANK)) ||
		...
		if (sprdbl.pwm_mode == dim_pwm) {
			sci_adi_clr((unsigned long)ANA_REG_GLB_WHTLED_CTRL0, BIT_WHTLED_BOOST_EN);
		} else {
		...
		if (sprdbl.pwm_mode == dim_pwm) {
			...
			reg_val = sci_adi_read((unsigned long)ANA_REG_GLB_WHTLED_CTRL1);
#ifdef CONFIG_ARCH_SCX15
			reg_val &= ~(0x7f << 6);
			reg_val |= led_level << 6;
#else
			...
			sci_adi_raw_write((unsigned long)ANA_REG_GLB_WHTLED_CTRL1, reg_val);
			#if 0
			/*dimming pwm config*/
			...
			reg_val = sci_adi_read((unsigned long)ANA_REG_GLB_WHTLED_CTRL0);
			reg_val &= ~(0x3f << 1);
			reg_val |= led_level << 1;
			sci_adi_raw_write((unsigned long)ANA_REG_GLB_WHTLED_CTRL0, reg_val);

			sci_adi_raw_write((unsigned long)(PD_PWM_BASE + PWM_SCALE), PWM_PRESCALE);
			sci_adi_raw_write((unsigned long)(PD_PWM_BASE + PWM_CNT),
					(pwm_level << 8) | PWM_MOD_MAX);

			sci_adi_raw_write((unsigned long)(PD_PWM_BASE + PWM_SCALE),
					PWM_SCALE | PWM_ENABLE);

			sci_adi_clr((unsigned long)ANA_REG_GLB_WHTLED_CTRL0, BIT_WHTLED_SERIES_EN);
			sci_adi_clr((unsigned long)ANA_REG_GLB_WHTLED_CTRL0, BIT_WHTLED_PD);
		}
	}
...
static struct resource *sprd_backlight_parse_dt(struct platform_device *pdev)
{
	...
	if (!pwm_res) {
		dev_err(&pdev->dev, "sprd_backlight Could not allocate struct resource");
		return NULL;
	}
	ret = of_property_read_u32(np, "start", &tmp_val_u32);
	if(ret){
		dev_err(&pdev->dev, "fail to get resource.start\n");
		goto fail;
	}
	pwm_res->start = tmp_val_u32;
	ret = of_property_read_u32(np, "end", &tmp_val_u32);
	if(ret){
		dev_err(&pdev->dev, "fail to get resource.end\n");
		goto fail;
	}
	pwm_res->end = tmp_val_u32;
	...
	if(ret){
		dev_err(&pdev->dev, "fail to get resource.flags\n");
		goto fail;
	}
	return pwm_res;
fail:
...
static int sprd_backlight_probe(struct platform_device *pdev)
{
	...
	sprintf(pwm_clk_name, "%s%d", "clk_pwm", (int)sprdbl.pwm_index);
